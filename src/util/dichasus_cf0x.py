...
with open(os.path.join(DATASET_DIR, "spec.json")) as specfile:
    ...
    print(len(spec["antennas"]))
    for antenna in spec["antennas"]:
        antenna_count = antenna_count + \
            sum([len(row) for row in antenna["assignments"]])
        ...
        normalvector = np.cross(np.asarray(
            antenna["rightvector"]), np.asarray(antenna["upvector"]))
...
def parse_and_calibrate(path, offset_path):
    ...
    def record_parse_function(proto):
        ...
        csi = tf.ensure_shape(tf.io.parse_tensor(
            record["csi"], out_type=tf.float32), (antenna_count, RAW_SUBCARRIER_COUNT, 2))
        expected_bytes = antenna_count * RAW_SUBCARRIER_COUNT * 2 * 4
        csi_length = tf.strings.length(record['csi'])
        # tf.print("CSI buffer length:", csi_length)
        ...
        # Phase increment normalization
        incr = tf.cast(tf.math.angle(tf.math.reduce_sum(
            csi[:, 1:] * tf.math.conj(csi[:, :-1]))), tf.complex64)
        csi = csi * \
            tf.exp(-1.0j * incr *
                    tf.cast(tf.range(csi.shape[-1]), tf.complex64))[tf.newaxis, :]

        # Parse position
        position = tf.ensure_shape(tf.io.parse_tensor(
            record["pos-tachy"], out_type=tf.float64), (3))
    ...
    def apply_calibration(csi, pos, time):
        ...
        sto_offset = tf.tensordot(
            tf.constant(offsets["sto"]), 2 * np.pi * tf.range(n, dtype=np.float32) / tf.cast(n, np.float32), axes=0)
        cpo_offset = tf.tensordot(
            tf.constant(offsets["cpo"]), tf.ones(n, dtype=np.float32), axes=0)
        phase = sto_offset + cpo_offset
        csi = tf.multiply(csi, tf.exp(tf.complex(0.0, phase)))
    ...
    def order_by_antenna_assignments(csi, pos, time):
        csi = tf.stack([[tf.gather(csi, antenna_indices)
                       for antenna_indices in array] for array in antenna_assignments])
        return csi, pos, time
...
def load_datasets(paths_list):
    full_dataset = parse_and_calibrate(
        paths_list[0]["tfrecords"], paths_list[0]["offsets"])

    for path in paths_list[1:]:
        full_dataset = full_dataset.concatenate(
            parse_and_calibrate(path["tfrecords"], path["offsets"]))
...
training_set_unlabeled = load_datasets(
    TRAINING_SET_UNLABELED).map(lambda csi, pos, time: (csi, time))

print(training_set_labeled)

# print(len(training_set_labeled))
# print(len(training_set_unlabeled))

# You may access the following variables for evaluation, but not for training
test_set = load_datasets(TEST_SET)

# --- DEBUG: Actually iterate to trigger tf.print and see CSI buffer length ---
if __name__ == "__main__":
    print("Iterating over a few samples to trigger tf.print...")
    for _ in training_set_labeled.take(5):
        pass
